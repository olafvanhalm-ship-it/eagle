#!/usr/bin/env python3
"""
Regression test for the ESMA 1.2 Input Adapter.
...
def _discover_test_cases() -> list[dict]:
    ...
    for f in sorted(_GOLDEN_SET.iterdir()):
        ...
        if suffix not in (".xml", ".gz", ".zip"):
            continue

        # Determine expected report type from filename
        ...
        # Determine expected RMS from filename (TYPE_RMS_...)
...
def _normalize_record(record: dict) -> str:
    ...
    Removes non-deterministic metadata fields.
    """
    clean = dict(record)
    # Remove fields that might change between runs
...
def run_test_case(case: dict) -> dict:
    ...
    try:
        ...
        # Validate completeness of record fields
        for i, rec in enumerate(adapter.records):
...
def save_baseline():
    ...
    baseline = {}
    print(f"\n{'='*60}")
    print(f"  ESMA Adapter — Saving Baseline")
    print(f"{'='*60}\n")
...
def run_tests():
    ...
    print(f"\n{'='*60}")
    print(f"  ESMA 1.2 Input Adapter — Regression Test")
    ...
    evidence = {
        "adapter": "ESMA 1.2 Input Adapter",
